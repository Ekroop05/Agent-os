...
class BuildOrchestrator:
    ...
    async def _run_pipeline(self, workspace_id: str, architecture: dict | None) -> None:
        ...
        try:
            ...
            foundation_tasks = [
                ("Create Project Folder", "Create the project root directory on disk.", "Critical"),
                ("Create Base Structure", "Create frontend/, backend/, docs/, .agentos/ directories.", "Critical"),
                ("Create README.md", "Generate project README with name, description, tech stack, and metadata.", "High"),
                ("Create Project Manifest", "Generate .agentos/project.json with full project metadata.", "High"),
            ]

            for title, desc, priority in foundation_tasks:
            ...
            while True:
                ...
                # Builder executes
                build_result = await builder_service.execute_task(next_task.id, workspace_id)

                if build_result["status"] == "error":
                    logger.error(f"Builder failed on task {next_task.id}: {build_result.get('error')}")
                    # Wait a moment and continue to next task
                    await asyncio.sleep(1)
                    # Skip this task - mark it failed
                    continue
                ...
                if not review_result.approved:
                    ...
                    if retry_count < MAX_RETRIES:
                        ...
                        await event_bus.publish(Event(
                            ...
                            severity="warning",
                            payload={"task_id": next_task.id, "retry": retry_count + 1},
                        ))
                        continue
                    else:
